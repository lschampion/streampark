...
# 使用mvn wrapper会导致使用/user/.m2/repository/作为本地仓库地址，无法使用mvn指定的仓库。所以修改为使用 本地mvn
# 以此方式引入修改的后的flink-kubernetes包。主要删除的所有的kubernetes所有的类。该为手动引入特定版本的kubernetes类
mvn -Pscala-2.11,shaded,webapp,dist -DskipTests -Dspotless.check.skip=true clean install
```
